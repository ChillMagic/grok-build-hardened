...
impl SessionActor {
    ...
    pub(super) async fn maybe_refresh_model_metadata_on_resume(&self) {
        // Privacy build: the server cannot change model metadata after startup.
    }
    ...
    pub(super) async fn handle_model_metadata_update(
        &self,
        _metadata: crate::sampling::ResponseModelMetadata,
    ) {
        // Privacy build: inference responses cannot mutate local model policy.
    }
